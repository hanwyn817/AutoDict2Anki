...
def process_word(word):
    ...
    try:
        ...
        if definition == "No definition available.":
            definition = formatted_word_data(word['uuid'], os.environ.get("AI_API_KEY", config.AI_API_KEY))
...
def job():
    ...
    try:
        cookie = os.environ.get("EUDICT_WEB_COOKIE", config.EUDICT_WEB_COOKIE)
        ...
        # 获取新增单词
        new_words = get_new_words(cookie, config.NEW_WORDS_PERIOD)
        ...
        for word in new_words:
            ...
            if can_add_card(word["uuid"], config.ANKI_DECK_NAME):
                ...
                success = False
                outcome = f"\n【重复】牌组 {config.ANKI_DECK_NAME} 中已存在与 {word["uuid"]} 相同的note，添加失败。"
